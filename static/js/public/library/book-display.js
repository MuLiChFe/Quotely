...
for (const filmId of idList) {
    try {
        const film_json = await getMarker("film", userId ,filmId);
        const film = film_json['marker'];
        // 创建电影卡片
        const filmCard = document.createElement('div');
        filmCard.classList.add('card', 'w-full', 'p-0');
        filmCard.innerHTML = `
            <div class="" data-bs-toggle="modal" data-bs-target="#bookModal" data-film-id="${film.film_id}">
...
;
    ...
    modal.addEventListener('show.bs.modal', async function (event) {
        ...
        try {
            const film_json = await getMarker("film",userId, currentFilmId); // 获取当前电影详情
            const film = film_json['marker'];
            // 更新模态框内容
            ...
            document.getElementById('film_id').textContent = film.film_id;
    ...
    addToLibraryBtn.addEventListener('click', async function () {
        const currentFilmId = document.getElementById('film_id').textContent; // 从模态框获取当前电影 ID
        try {
