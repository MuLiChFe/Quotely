...
class TeamMember(models.Model):
    member = models.ForeignKey('User', on_delete=models.CASCADE, null=True,blank=True,related_name='belonging_team')

    role = models.CharField(choices=[('teacher', 'Teacher'), ('student', 'Student')], max_length=20, default='student')
    belong_workspace = models.ForeignKey(Workspace,on_delete=models.CASCADE, related_name='team_members', blank=True, null=True)

    def __str__(self):
        return f'{self.belong_workspace.name} : {self.member.username} - {self.role}'
